<template>
  ...
  <form @submit.prevent="" class="space-y-10">
    ...
    <div class="flex justify-center mt-4">
      <button style="background-color: #098071 !important;border-radius: 10px;color:white" class="w-full lg:w-[50%] py-2 text-lg" @click="authStore.handleLogin(form)">{{ t('auth.login') }}</button>
    </div>
  </form>
</template>
